...
describe('nethernet lifecycle and RakNet compatibility', () => {
  ...
  it('does not initialize a client closed during discovery', async () => {
    ...
    client.close()
    discovery.resolve({ version: 7, gameVersion: '1.21.0', protocol: Versions['1.21.0'] })
    await new Promise(resolve => setImmediate(resolve))
  ...
  it('reports initialization errors without retrying with the fallback', async () => {
    ...
    let initialized = 0
    stub(NethernetClient.prototype, 'ping', async () => ({ version: 7, gameVersion: '1.21.0', protocol: Versions['1.21.0'] }))
  ...
  for (const transport of ['raknet', 'nethernet']) {
    const cases = [
      ...
      ['1.21.0.3', Versions['1.21.0'], undefined, '1.21.0'],
      ['9.99.0', Versions['1.21.0'], undefined, '1.21.0'],
      ['1.26.51', Versions['1.21.0'], undefined, '1.21.0'],
      [undefined, Versions['1.21.0'], undefined, '1.21.0'],
      ['1.26.51', 99999, undefined, undefined],
      ['9.99.0', 99999, '1.21.0', '1.21.0'],
      [undefined, undefined, undefined, CURRENT_VERSION]
    ]
    for (const [advertised, protocol, explicit, expected] of cases) {
      it(`selects ${transport} protocol ${protocol}, advertised=${advertised}, explicit=${explicit}`, async () => {
        const initialized = deferred()
        if (transport === 'raknet') {
