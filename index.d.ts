...
declare module 'bedrock-protocol' {
  ...
  export interface NethernetOptions {
    ...
    // Remote ID for clients (discovered by createClient when omitted); local ID for servers.
  ...
  export interface Options {
    ...
    // createClient discovers both when omitted; low-level Client and servers default to RakNet.
  ...
  export interface ClientOptions extends Options {
    ...
    // Advertisement lookup: 1000ms for automatic/RakNet discovery, 10000ms for explicit Nethernet.
    pingTimeout?: number
  ...
  export interface PingOptions {
    ...
    port?: number
    transport?: 'raknet' | 'nethernet'
    nethernet?: Pick<NethernetOptions, 'networkId' | 'webrtcBackend'>
    timeout?: number
    signal?: AbortSignal
  }

  export function ping(options: PingOptions & { transport: 'raknet' }): Promise<ServerAdvertisement & { transport: 'raknet' }>
  export function ping(options: PingOptions & { transport: 'nethernet' }): Promise<NethernetServerAdvertisement & { transport: 'nethernet' }>
  export function ping(options: PingOptions & { transport?: undefined, nethernet: NonNullable<PingOptions['nethernet']> }): Promise<NethernetServerAdvertisement & { transport: 'nethernet' }>
