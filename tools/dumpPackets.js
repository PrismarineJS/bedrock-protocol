...
async function dump (version) {
  ...
  const handle = await vanillaServer.startServerAndWait(
    ...
    async res => {
      ...
      client.on('packet', async packet => {
        ...
        if (!kindCounter[packet.name]) {
          await fs.promises.mkdir(path.join(output, 'from-server', name), {
            recursive: true
          })
          kindCounter[name] = 0
        }
        ...
        await fs.promises.writeFile(
          path.join(output, 'from-server', name, `${kindCounter[name]}.bin`),
          fullBuffer
        )

        try {
          fs.writeFileSync(
            path.join(output, 'from-server', name, `${kindCounter[name]}.json`),
            serialize(params, 2)
          )
        } catch (e) {
          console.log(e)
...
async function makeStats (kindCounter, version) {
  const { collectedPackets, allPackets } = parsePacketCounter(
    version,
    kindCounter
  )
  // write packet data
  ...
  await fs.promises.writeFile(
    path.join(metadataFolder, 'packets_info.json'),
